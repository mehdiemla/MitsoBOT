...
VPN_RATE_TIERS = {
    "1_5": {"label": "1 تا 5 گیگ", "default": 110_000},
    "5_10": {"label": "بیشتر از 5 تا 10 گیگ", "default": 110_000},
    "10_15": {"label": "بیشتر از 10 تا 15 گیگ", "default": 110_000},
    "15_20": {"label": "بیشتر از 15 تا 20 گیگ", "default": 110_000},
    "20_plus": {"label": "بیشتر از 20 گیگ", "default": 110_000},
}
...
def products_by_subcategory_keyboard(category_key: str, subcategory_key: str) -> InlineKeyboardMarkup:
    buttons = [
        ...
        for key, item in config.PRODUCTS.items()
        if item["category"] == category_key and item.get("subcategory") == subcategory_key
    ]
    rows = button_rows(buttons, 2)
    rows.append(
        [
...
def subcategory_products_text(category_key: str, subcategory_key: str) -> str:
    ...
    for product_key, product in products:
        if product.get("variable_volume"):
            lines.append(f"• {product['title']}: هر گیگ 99,000 تومان، حجم دلخواه")
        else:
            lines.append(f"• {product['title']}: {toman(product_price(product_key))}")
    return "\n".join(lines)
...
def is_navigation_text(text: str) -> bool:
    ...
    if text in {
        ...
        return True
    return any(text == product["title"] for product in config.PRODUCTS.values())
...
async def start_product_order(update: Update, context: ContextTypes.DEFAULT_TYPE, product_key: str) -> int:
    ...
    await respond_in_place(
        update,
        f"انتخابت ثبت شد: {product['title']}\n"
        f"قیمت: {toman(price)}"
        f"{description_text}\n\n"
...
def format_order_for_admin(order, payment_label: str | None = None) -> str:
    ...
    if order["spotify_password"]:
        if "🔐 اطلاعات اکانت" not in lines:
            lines.extend(["", "🔐 اطلاعات اکانت"])
        lines.append(f"پسورد اکانت: <code>{escape(order['spotify_password'])}</code>")
